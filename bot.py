...
def register_telegram_commands(target_bot=None):
    ...
    commands = [
        ...
        types.BotCommand(
            "workspace",
            "📂 Change server working directory (synced to AI)",
        ),
    ...
    try:
        ...
        print(
            "✅ ALL 18 Telegram Slash Commands registered successfully into "
            "dropdown menu!",
        )
        return True
...
def check_auth(func):
    ...
    def wrapper(message, *args, **kwargs):
        ...
        print(
            f"[RECV MSG] From User ID: {user_id} (@{user_name}) - "
...
def send_long_message(  # noqa: C901
    chat_id: int,
    text: str,
    reply_markup=None,
) -> types.Message | None:
...
def keep_typing_alive(chat_id: int, stop_event: threading.Event) -> None:
...
def show_model_picker(message_or_call):
    ...
    for m in models:
        ...
        markup.add(
            types.InlineKeyboardButton(btn_text, callback_data=f"set_model:{m_id}"),
        )

    btn_eff = types.InlineKeyboardButton(
        "🎯 Change Effort Level (/effort)",
        callback_data="open_effort_menu",
    )
    markup.add(btn_eff)
...
def show_effort_picker(message_or_call):
    ...
    for eff_key, eff_name in efforts:
        ...
        markup.add(
            types.InlineKeyboardButton(btn_text, callback_data=f"set_effort:{eff_key}"),
        )
...
def show_mode_picker(message_or_call):
    ...
    for mode_key, mode_name in modes:
        ...
        markup.add(
            types.InlineKeyboardButton(btn_text, callback_data=f"set_mode:{mode_key}"),
        )
...
def render_file_explorer(message_or_call, path_dir):  # noqa: C901
    chat_id = (
        message_or_call.chat.id
        if hasattr(message_or_call, "chat")
        else message_or_call.message.chat.id
    ...
    if parent and parent != norm_path:
        ...
        markup.add(
            types.InlineKeyboardButton(
                "⬆️ .. (Parent Directory)",
                callback_data=f"browse_dir:{up_token}",
            ),
        )
    ...
    if os.path.abspath(norm_path) != os.path.abspath(cur_ws):
        ...
        markup.add(
            types.InlineKeyboardButton(
                "📍 Set as AI Target Workspace",
                callback_data=f"set_ws:{set_token}",
            ),
        )
    ...
    try:
        ...
        for d in dirs[:10]:
            ...
            markup.add(
                types.InlineKeyboardButton(
                    f"📁 {d}/",
                    callback_data=f"browse_dir:{token}",
                ),
            )
        ...
        for fname, sz in files[:10]:
            markup.add(
                types.InlineKeyboardButton(
                    f"📄 {fname} ({sz} KB)",
                    callback_data=f"info_file:{fname[:20]}",
                ),
            )
...
def show_active_session_info(message):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "▶️ Select Another Session",
            callback_data="select_session_menu",
        ),
        types.InlineKeyboardButton(
            "🔄 New Session (/new)",
            callback_data="select_session|new",
        ),
    )
...
def show_session_picker(message_or_call):
    ...
    for sess in sessions:
        ...
        btn = types.InlineKeyboardButton(
            btn_text,
            callback_data=f"select_session|{sess['id']}",
        )
        markup.add(btn)
...
def delete_session_command(message):
    ...
    for sess in sessions:
        ...
        btn = types.InlineKeyboardButton(
            btn_text,
            callback_data=f"delete_session|{sess['id']}",
        )
        markup.add(btn)
...
def show_workspace_picker(message_or_call):
    ...
    for d in available_dirs:
        ...
        is_active = (
            " (Active)" if os.path.abspath(d) == os.path.abspath(current_ws) else ""
        )
        btn_text = f"📁 {folder_name}{is_active}"
        token = path_mapper.encode(d)
        markup.add(
            types.InlineKeyboardButton(btn_text, callback_data=f"set_ws:{token}"),
        )

    btn_tree = types.InlineKeyboardButton(
        "🌳 Interactive File Explorer (/tree)",
        callback_data="open_tree_explorer",
    )
    markup.add(btn_tree)
...
def handle_media_prompt(message):
    try:
        caption = (
            message.caption
            or "Analyze this file/photo and help fix any errors if present."
        )
        file_info = None
...
def process_custom_agent_prompt(  # noqa: C901
    message,
    prompt,
    status_text,
    runner_func,
